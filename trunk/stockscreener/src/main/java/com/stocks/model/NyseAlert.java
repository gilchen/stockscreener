...
@NamedQueries({
	...
	@NamedQuery(name = "allNyseAlertResultsByTrxType", query = "select new com.stocks.search.NyseAlertResult(a, b) from NyseAlert a, Nyse b where a.symbol=b.nysePK.symbol and b.nysePK.tradeDate=(select max(c.nysePK.tradeDate) from Nyse c) and ((a.isActive in ('Y', 'N') and :isActive=null) or a.isActive=:isActive) and a.trxType=:trxType order by a.symbol, a.eventDate")
})
